...
const schemaFile = ['models.js', 'resolvers.js'];

const start = async () => {
   if (!process.argv[2]) {
      console.log(
         `Please specify name to make schema. Example: yarn make:schema User`
      );
      process.exit(1);
   }
   ...
   let collectionName = readlineSync.question(
      `Please enter collection name in ${schemaName}Model (ex: Users) `
   );
   ...
   if (continueGenerate) {
      ...
      console.log(
         'Select schema type lists: ex: foo(String) or foo for default type string\n'
      );
      ...
      for (let i = 1; i <= totalField; i++) {
         let fieldName = readlineSync.question(
            `${i}). Enter field name! (ex: foo(String) ) `
         );
   ...
   for (const file of schemaFile) {
      writeFileSync(join(schemaPath, file), `const ${schemaName};`);
   }
   ...
   lineReplace({
      ...
      callback: ({ file, line, text, replacedText, error }) => {
         if (error) {
            ...
            console.log(
               `Successfully make ${schemaName} models${
                  fieldLists[0] !== undefined
                     ? ` with field ${fieldLists.join(', ')}`
                     : ''
               }!`
            );
         }
...
const ${schemaName} = Builder.model('${schemaName}', ${_.camelCase(
      schemaName
   )}Schema);
